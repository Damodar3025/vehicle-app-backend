  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        ...
        { ...form, numberOfTons: Number(numberOfTons), amount: Number(amount) },
    ...
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subGreeting}>Submit your vehicle details</Text>
        </View>
      ...
      <View style={styles.infoCard}>
        ...
        <Text style={styles.infoValue}>{user?.vehicleType}</Text>
      </View>
      ...
      <View style={styles.formCard}>
        ...
        <TextInput
          ...
          value={form.numberOfTons}
          onChangeText={(v) => handleChange('numberOfTons', v)}
        />
      ...
      {lastSubmission && (
        <View style={styles.successCard}>
          ...
          <Text style={styles.successText}>SMS: {lastSubmission.smsSent ? '✅ Sent' : '❌ Not sent'}</Text>
        </View>
      )}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  infoCard: { backgroundColor: '#4f46e5', borderRadius: 14, padding: 18, marginBottom: 20 },
  infoLabel: { color: '#c7d2fe', fontSize: 12, marginTop: 8 },
  infoValue: { color: '#fff', fontSize: 16, fontWeight: '700' },
  formCard: { backgroundColor: '#fff', borderRadius: 14, padding: 20, marginBottom: 20, elevation: 2, shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 8 },
  ...
  input: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10, padding: 13, fontSize: 15, color: '#333', backgroundColor: '#fafafa' },
  dropdown: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10, padding: 13, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fafafa' },
  ...
  button: { backgroundColor: '#4f46e5', borderRadius: 10, padding: 16, alignItems: 'center', marginTop: 24 },
  buttonText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  successCard: { backgroundColor: '#f0fdf4', borderWidth: 1, borderColor: '#86efac', borderRadius: 14, padding: 18 },
